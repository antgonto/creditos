...
namespace BLL
{
    public class Precio_Producto
    {
        ...
        private string _tipo_precio_cod_precio;
        public string tipo_precio_cod_precio
        {
            get { return _tipo_precio_cod_precio; }
            set { _tipo_precio_cod_precio = value; }
        }
        ...
        #region metodos
        public string cargar_lista_precio_producto(string cod_prod)
        {
        ...
        public bool agregar_Precio_a_Producto(string accion)
        {
            conexion = cls_DAL.trae_conexion("Progra5", ref mensaje_error, ref numero_error);
            if (conexion == null)
            {
            ...
            else
            {
                ...
                {
                    sql = "stp_precio_prod_crear";
                }
                ParamStruct[] parametros = new ParamStruct[4];
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 0, "@tipo_precio_cod_precio", SqlDbType.VarChar, _tipo_precio_cod_precio);
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 1, "@producto_cod_prod", SqlDbType.VarChar, _producto_cod_prod);
        ...
        public bool modificar_precio_prod(string accion)
        {
            conexion = cls_DAL.trae_conexion("Progra5", ref mensaje_error, ref numero_error);
            if (conexion == null)
            {
            ...
            else
            {
                ...
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 2, "@tipo_precio", SqlDbType.VarChar, _tipo_precio_cod_precio);
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 3, "@usuario_nom_usu", SqlDbType.VarChar, _usuario_nom_usu);
                cls_DAL.conectar(conexion, ref mensaje_error, ref numero_error);
                cls_DAL.ejecuta_sqlcommand(conexion, sql, true, parametros, ref mensaje_error, ref numero_error);
                if (numero_error != 0)
        ...
        public bool eliminar_precio_producto(string cod_prod, string cod_precio)
        {
            conexion = cls_DAL.trae_conexion("Progra5", ref mensaje_error, ref numero_error);
            if (conexion == null)
            {
